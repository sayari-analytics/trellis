...
import { TextureTable, createTextureTable, stylesGLSL, red, green, blue, alpha, NODE_STYLE_UNIT, EDGE_STYLE_UNIT } from '.'
...
const DEFAULT_NODE_STYLE: Required<Omit<NodeStyle, 'label' | 'icon'>> = { fillColor: 0x3380e6, strokeWidth: 2, strokeColor: 0x194080 }
const DEFAULT_EDGE_STYLE: Required<Pick<EdgeStyle, 'fillColor'>> = { fillColor: 0x99a3ad }
...
// texel 0 = fill rgba, texel 1 = stroke rgba, texel 2 .x = strokeWidth. colors carry their own alpha
// (0xAARRGGBB), so a translucent fill/stroke needs no separate opacity field.
...
const packEdgeStyle = (data: Float32Array, o: number, s: EdgeStyle | undefined) => {
  ...
  data[o + 1] = green(fillColor)
  data[o + 2] = blue(fillColor)
  data[o + 3] = alpha(fillColor) // texel 0 .a is the fill alpha; edge width is per-edge data, not packed here
...
// texel 1 = (positionCode, margin, _, _); texel 2 = (strokeColor.rgb, strokeWidth). Both label programs
