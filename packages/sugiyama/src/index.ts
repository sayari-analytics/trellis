 ...
 * a column (longest-path layering), orders nodes within each column to minimize edge crossings (barycenter
...
export type NodeLayout = {
  layer: number // column index, 0 = source-most
  order: number // index within its column, top-most first
...
export type EdgeLayout = {
  ...
  // laid-out endpoint positions (post anchor + centroid transform) + stacked attachment offset (0 until
...
export type Options<N extends Node, E extends Edge> = Partial<{
  ...
  anchor: 'bottom' | 'left' | 'top' | 'right' // direction the flow grows (left = flow rightward)
  align: Align // column assignment: 'left' (default), 'right', or 'justify'
  nodeSize: [number, number] // [gap between columns, gap between nodes within a column]
  ...
  // hierarchy layout's `size` option (overrides nodeSize). Omitted until the extents-and-scale math is
  // worked out against the extent-aware coordinate assignment.
}>

const DEFAULT_NODE_SIZE: [number, number] = [200, 40]
...
export const Layout = () => {
  return <N extends Node, E extends Edge>({
    ...
    const index = createDirectedIndex(graph)
    const backEdges = findBackEdges(index, graph.nodes)

    const { layer: layerOf, maxLayer } = assignLayers(index, graph.nodes, backEdges, options.align ?? 'left')
    const layered = buildLayeredGraph(index, graph.nodes, graph.edges, layerOf, maxLayer, linkValue)
    orderVertices(index, layered, maxLayer, options.iterations ?? DEFAULT_ITERATIONS, options.sort)

    const [columnGap, rowGap] = options.nodeSize ?? DEFAULT_NODE_SIZE
    const coords = assignCoordinates(index, layered, nodeBreadth, columnGap, rowGap)
    ...
    // Map raw layout coordinates onto world x/y per anchor; +y is up in the renderer.
    ...
    const positioned = graph.nodes.map((node) => ({ node, ...toWorld(coords.get(nodeKey(node.id))!) }))
    ...
    for (let i = 0; i < count; i++) {
      inputCx += graph.nodes[i].x
      inputCy += graph.nodes[i].y
