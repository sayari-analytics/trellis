...
export class NodeRenderer {
  ...
  update(node: Graph.Node) {
    ...
    if (
      ...
      this.renderer.animateNodePosition && this.renderer.renderedNodes
    ) {
  ...
  private setPosition(node: Graph.Node, x: number, y: number, radius: number) {
    ...
    if (this.label && node.label) {
      const labelPosition = positionNodeLabel(this.x, this.y, this.strokes.radius, node.style?.label?.orientation)
  ...
  private visible() {
    ...
    return (this.x + this.strokes.radius) >= this.renderer.minX &&
