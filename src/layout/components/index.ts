...
export type ComponentsOptions = Partial<{ padding: number }>

const DEFAULT_PADDING = 18

export const Components = {
  Layout: () => {
    return <N extends Node, E extends Edge>(graph: { nodes: N[]; edges: E[]; options?: ComponentsOptions }) => {
      ...
      return packSiblings(components).reduce<{ nodes: N[]; edges: E[] }>(
        (graph, { x0, y0, x, y, nodes, edges }) => {
          const xOffset = x0 - x
          const yOffset = y0 - y
