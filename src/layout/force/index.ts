...
const workerScript = (DEFAULT_OPTIONS: typeof DEFAULT_LAYOUT_OPTIONS) => {
  const layout = ({
    nodes,
    edges,
    options: {
...
const blob = new Blob([`${d3ForceScript}(${workerScript})(${JSON.stringify(DEFAULT_LAYOUT_OPTIONS)})`], {
...
export const Force = {
  Layout: () => {
    ...
    const layout = <N extends Node, E extends Edge>(graph: { nodes: N[]; edges: E[]; options?: ForceOptions }) => {
