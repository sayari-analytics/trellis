...
const App: FunctionComponent = () => {
  ...
  useEffect(() => {
    force({ nodes: data.nodes, edges: data.edges }).then(({ nodes, edges }) =>
      setGraph((graph) => ({ ...graph, nodes, edges })),
    )
  }, [])
  ...
  const onNodeDrag = useCallback(
    ({ nodeX, nodeY, target: { id, x = 0, y = 0 } }: WebGL.NodeDragEvent) => {
      const dx = nodeX - x
      const dy = nodeY - y
  ...
  const onNodePointerUp = useCallback(
    ({ metaKey, shiftKey, target: { id } }: WebGL.NodePointerEvent) => {
      setGraph((graph) => ({
        ...graph,
        selected:
  ...
  const onViewportDrag = useCallback(
    ({
      viewportX: x,
      viewportY: y,
    }: WebGL.ViewportDragEvent | WebGL.ViewportDragDecelerateEvent) => {
      setGraph((graph) => ({ ...graph, x, y }))
    },
    [],
  )
  const onViewportWheel = useCallback(
    ({ viewportX: x, viewportY: y, viewportZoom: zoom }: WebGL.ViewportWheelEvent) => {
    ...
    return graph.edges.map<Graph.Edge>((edge) => ({
      ...
      style:
        edge.id === graph.hoverEdge
          ? { width: 3, arrow: 'forward' as const }
          ...
          children: ({
            ...
            createElement(
              ...
              createElement(
                ...
                createElement(
                  Button,
                  { title: 'Select Tool', selected: select, onClick: toggleSelect },
                  '■',
                ),
