...
const nodes = raw.nodes.map((id, idx) => ({ id: `${id}`, radius: 18, style: idx % 4 === 0 ? NODE_STYLE_A : NODE_STYLE_B }))
const edges = raw.edges.map(([source, target], idx) => ({ id: `${idx}`, source: `${source}`, target: `${target}`, style: EDGE_STYLE }))


const filterGraph = (predicate: (node: Graph.Node) => boolean, graph: { nodes: Graph.Node[], edges: Graph.Edge[] }) => {
  const nodeMap = graph.nodes.reduce<Record<string, Graph.Node>>((nodeMap, node) => (nodeMap[node.id] = node, nodeMap), {})
  return {
...
Promise.all<{ nodes: Graph.Node[], edges: Graph.Edge[] }>([
  force(filterGraph(() => true, { nodes, edges })),
