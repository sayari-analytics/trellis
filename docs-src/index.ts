...
const layouts = raw.map(({ roots, nodes, edges }) => {
  return {
    roots,
    nodes: nodes.map(([id, x, y], idx) => ({ id: `${id}`, x, y, radius: 18, label: `${idx % 4 === 0 ? 'person' : 'company'} ${id}`, style: idx % 4 === 0 ? NODE_STYLE_A : NODE_STYLE_B })),
    edges: edges.map(([id, source, target]) => ({ id: id as string, source: `${source}`, target: `${target}`, label: 'linked to', style: EDGE_STYLE })),
  }
})
