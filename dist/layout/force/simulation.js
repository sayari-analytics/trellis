...
var workerScript = function (DEFAULT_OPTIONS) {
    ...
    var Simulation = /** @class */ (function () {
        function Simulation() {
            ...
            this.nodesById = {};
            this.subGraphs = {};
            this.forceLink = d3.forceLink().distance(this.options.linkDistance).id(function (node) { return node.id; });
            this.forceManyBody = d3.forceManyBody().strength(this.options.nodeStrength).distanceMax(4000).theta(0.5);
            this.forceCollide = d3.forceCollide().radius(function (node) { return node.radius + _this.options.nodePadding; });
            this.forceX = d3.forceX(0).strength(this.options.centerStrength);
            this.forceY = d3.forceY(0).strength(this.options.centerStrength);
            ...
            this.fisheyeCollapse = function (nodes, subGraphs) {
                ...
                for (var i = subGraphsById.length - 1; i >= 0; i--) {
                    ...
                    x = _this.nodesById[id].x; // TODO - x/y position should be a property on each subGraph simulation (as should radius)
                    y = _this.nodesById[id].y; // TODO - x/y position should be a property on each subGraph simulation (as should radius)
                    radius = 200;
            ...
            this.fisheyeExpand = function (nodes, subGraphs) {
                ...
                for (var i = 0; i < subGraphsById.length; i++) {
                    id = subGraphsById[i][0];
                    x = _this.nodesById[id].x;
                    y = _this.nodesById[id].y;
                    radius = 200;
            ...
            this.postLayout = throttle(function () {
                self.postMessage({ nodes: simulation.simulation.nodes() });
            });
        }
        Simulation.prototype.layout = function (nodes, edges, _a) {
            var _b = _a === void 0 ? DEFAULT_OPTIONS : _a, _c = _b.nodeStrength, nodeStrength = _c === void 0 ? DEFAULT_OPTIONS.nodeStrength : _c, _d = _b.linkDistance, linkDistance = _d === void 0 ? DEFAULT_OPTIONS.linkDistance : _d, _e = _b.linkStrength, linkStrength = _e === void 0 ? DEFAULT_OPTIONS.linkStrength : _e, _f = _b.centerStrength, centerStrength = _f === void 0 ? DEFAULT_OPTIONS.centerStrength : _f, _g = _b.nodePadding, nodePadding = _g === void 0 ? DEFAULT_OPTIONS.nodePadding : _g, _h = _b.tick, tick = _h === void 0 ? DEFAULT_OPTIONS.tick : _h;
            ...
            for (var i = 0; i < nodes.length; i++) {
                ...
                if (node.subGraph) {
                    if (this.subGraphs[node.id] === undefined) {
                        // enter subgraph
                        subGraphs[node.id] = new Simulation().layout(node.subGraph.nodes, node.subGraph.edges, node.subGraph.options === undefined ? this.defaultSubgraphOptions : node.subGraph.options);
                    }
                    else {
                        // update subgraph
                        subGraphs[node.id] = this.subGraphs[node.id].layout(node.subGraph.nodes, node.subGraph.edges, node.subGraph.options === undefined ? this.defaultSubgraphOptions : node.subGraph.options);
                    }
        ...
        Simulation.prototype.update = function (nodes) {
            for (var i = 0; i < nodes.length; i++) {
                var node = nodes[i];
