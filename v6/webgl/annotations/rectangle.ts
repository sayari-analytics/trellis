...
import { RectangleAnnotation, TextAnnotation } from '../../../src'
  ...
  private resizePointerMove = (hitBoxIdx: number) => (event: PIXI.InteractionEvent) => {
    ...
    if (this.annotation.type === 'text') {
      const style = new PIXI.TextStyle({
        ...
        //account for padding
        wordWrapWidth:
          this.annotation.style.text?.maxWidth ?? this.annotation.width - 2 * (this.annotation.style.padding ?? DEFAULT_PADDING) ?? 0,
        breakWords: true,
